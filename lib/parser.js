...
var Parser = function(fight, actorID, friendlies, events, ignoreFriendlies) {
    ...
    self.events = events.filter(function(event) {
        ...
        // filter out buffs that aren't heals we care about
...
Parser.prototype._processCritContribution = function(event) {
    ...
    var hpsPerPoint = (critHealingAmount / critTotalAmount) * critGearRatio;

    return { healingFromCrit: critHealingAmount, hpsPerPoint: hpsPerPoint };
}
...
Parser.prototype._processMasteryContribution = function(event) {
    ...
    var hpsPerPoint = healingByGearMastery / mastery.gear;

    return { healingFromMastery: healingByGearMastery, hpsPerPoint: hpsPerPoint, masteryEffectiveness: masteryEffectiveness };
}
...
Parser.prototype._applyBuffEvent = function(event) {
    ...
    var idx = null;
    self.targets[event.targetID].heals.forEach(function(_ability, _idx) {
    ...
    // if there were any heals on the target before this HoT was applied
    //  then we attribute the time since the previous HoT was applied / expired to the stack count
    if (self.targets[event.targetID].heals.length > 0) {
        var stacks = self.targets[event.targetID].heals.length;
...
Parser.prototype._applyHoT = function(timestamp, targetID, ability) {
    ...
    self.targets[targetID].heals.push(ability);
};
...
Parser.prototype._removeBuffEvent = function(event) {
    ...
    if (idx === null) {
        ...
        //  but then having this HoT already on the target (from pre-heals)
        if (REPARSE) {
            throw new ReparseError(event);
        }

        // ignore errors for heals that could have been applied before combat started
...
Parser.prototype._removeHoT = function(timestamp, targetID, ability, idx) {
    ...
    self.targets[targetID].heals.forEach(function(_ability, _idx) {
    ...
    // then add our new HoT to the target
    self.targets[targetID].timeLastChanged = timestamp;
    self.targets[targetID].heals.splice(idx, 1);
};
