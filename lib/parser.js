...
var Parser = function(fight, actorID, friendlies, events, ignoreFriendlies) {
    ...
    self.talents = combatantInfo.talents.map(function(talent) {
        return talent.id;
    });
    ...
    var tier19 = combatantInfo.gear.filter(function(item) {
        return consts.TIER19IDS.indexOf(item.id) !== -1;
    });
...
Parser.prototype.result = function() {
    ...
    return {
        ...
        sb: self.talents.indexOf(consts.TALENTS.SB) !== -1,
...
Parser.prototype._healEvent = function(event) {
    ...
    var isTearstoneRejuv = self.targets[event.targetID].tearstoneRejuv && event.ability.name === SPELLS.REJUV;
    var isTier194pcRejuv = self.targets[event.targetID].tier194pcRejuv && event.ability.name === SPELLS.REJUV;
    ...
    if (HOTS.indexOf(event.ability.name) !== -1 && typeof self.targets[event.targetID].hotsFirstTick[event.ability.name] === "undefined") {
        self.targets[event.targetID].hotsFirstTick[event.ability.name] = event.timestamp;
    }
    ...
    self.targets[event.targetID].hots.forEach(function (_ability, _idx) {
        if (event.ability.name !== _ability.name) {
            self.masteryStacksHealingPerHoT[_ability.name] += (event.amount * self.masteryPercentage());
        }
    });
...
Parser.prototype._applyOrRefreshBuffEvent = function(event, isRefreshBuff) {
    ...
    // @TODO: germ
    if (event.ability.name === SPELLS.REJUV) {
        self.targets[event.targetID].tearstoneRejuv = tearstoneRejuv;
        self.targets[event.targetID].tier194pcRejuv = tier194pcRejuv;
    }
};
