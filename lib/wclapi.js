 ...
 * @returns q.promise
 */
WCLAPI.prototype.getClasses = function() {
    var self = this;

    return self.requestPromise("/classes", {
        api_key: self.apikey
    })
        .then(function(body) {
...
WCLAPI.fixEvents = function(events, actorID, start_time) {
    var s = null, e = null;
    // s = -1.0; e = 2.0;
    // s = 25.0; e = 26.0;
    // s = 47.0; e = 48.0;
    ...
    var BLACKLIST_GUIDS = [201633, 214583];
    ...
    events = events.filter(function (event) {
        ...
        // these events we only want if the source is our actor
        if ([
                'heal',
                'cast',
                'applybuff', 'removebuff', 'refreshbuff'
            ].indexOf(event.type) !== -1 && event.sourceID == actorID) {
            return true;
        }
    });

    events = events.filter(function (event) {
        return !event.ability || BLACKLIST_GUIDS.indexOf(event.ability.guid) === -1;
    });
...
WCLAPI.prototype.getEvents = function(report, actorid, start_time, end_time) {
    ...
    return _getActorEvents(start_time)
        ...
        .then(function(events) {
            ...
            return WCLAPI.fixEvents(events, actorid, start_time);
        });
};
